...
class Solver(object):
    ...
    def model_init(self, args):
        ...
        # self.net = torchvision.models.vgg19().cuda()
    ...
    def train(self):
        ...
        for e in range(self.epoch):
            ...
            for batch_idx, (images, labels) in enumerate(self.data_loader['train']):
                self.global_iter += 1
                #print(images.shape) # 100, 3, 32, 32 => batch_size, nc, h, w
                ...
                if batch_idx % 100 == 0:
                    ...
                    if self.tensorboard:
                        ...
                        self.tf.add_scalars(main_tag='performance/cost',
                                            tag_scalar_dict={'train':cost.data.item()},
                                            global_step=self.global_iter)
    ...
    def generate(self, num_sample=128, target=-1, epsilon=0.03, alpha=2/255, iteration=1):
        self.set_mode('eval')

        # if mnist -->
        x_true, y_true = self.sample_data(num_sample = self.batch_size)
        # sample_data is not functional for cifar
    ...
    def sample_data(self, num_sample=100):
        ...
        # import numpy as np
        # seed = [np.random.random_integers(1,total) for _ in range(num_sample)]
        x = torch.from_numpy(self.data_loader['test'].dataset.test_data[seed]).float()
        print(x.shape)
        # x = self.scale(torch.from_numpy(x).float().unsqueeze(1).div(255))
        # print(type(x)) #torch_tensor
        y = [self.data_loader['test'].dataset.test_labels[i] for i in seed]
